...
const aStar = async (from, to) => {
    ...
    while (true) {
        ...
        const minorHeuristic = Math.min.apply(null, line.map(el => el.heuristic + el.cost)); // Find the minor Heuristic
        swap(line.findIndex(el => el.heuristic + el.cost === minorHeuristic)); // Change him to first
    }
}
